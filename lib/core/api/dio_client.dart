...
/// Factory pour créer un client Dio configuré.
///
/// Le client est configuré avec :
/// - Intercepteur JWT (Firebase ID token injecté automatiquement)
/// - Retry automatique sur erreur réseau (3 tentatives max)
/// - Certificate pinning (production uniquement)
/// - Logging en debug
  ...
  static Dio create() {
    ...
    dio.interceptors.addAll([
      ...
      if (kDebugMode)
        LogInterceptor(
          requestBody: true,
          responseBody: false, // Ne jamais loguer les corps de réponse (privacy)
          error: true,
        ),
    ]);

    // Certificate pinning en production (sha256 du certificat backend)
    if (kReleaseMode) {
      dio.httpClientAdapter = IOHttpClientAdapter(
        createHttpClient: () {
          final client = HttpClient();
          // TODO: activer le pinning une fois le certificat prod connu
          // client.badCertificateCallback = (cert, host, port) {
          //   return cert.pemFingerprint == ApiConfig.expectedCertFingerprint;
          // };
          return client;
        },
      );
...
class _AuthInterceptor extends Interceptor {
  ...
  Future<void> onRequest(
    ...
    // Ne pas bloquer les requêtes si le token n'est pas encore prêt
    try {
...
/// Provider singleton pour le Dio client.
