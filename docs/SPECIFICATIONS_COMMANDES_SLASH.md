...
`api.zentic.fr` n'est pas déployé. Or, **7 commandes** (`scrape`, `crawl`, `download`, `links`, `summarize`, `extract`, `metadata`, `export`) dépendent du backend pour leur fonctionnement avec URL. Le code Dart vérifie :
```dart
...
│  │ Bridge      │  │ api.zentic.fr   │  │ (Python/Node.js)   │ │
│  │ (DOM/API)   │  │ (FastAPI)        │  │                    │ │
│  └──────┬──────┘  └────────┬─────────┘  └─────────┬──────────┘ │
│         │                  │                       │            │
│         ▼                  ▼                       ▼            │
...
2. **Chemin cloud** : Backend `api.zentic.fr` → scraping/crawling
...
**Problème** : `api.zentic.fr` non déployé.
...
Le backend `api.zentic.fr` est le nœud critique. Sans lui, 8 commandes sur 26 (~31%) sont inopérantes en mode « URL distante ».
...
| 6 | Configurer `BACKEND_URL=api.zentic.fr` dans les builds | Production ready |
...
2. **Backend cloud** (`api.zentic.fr`) comme second niveau, déployé sur Fly.io (gratuit) ou via le script Docker existant. Il offre :
